...
public class HomeWork2 {
    ...
    public static void validateLogin(String login) throws LoginValidationException {
        for (int i = 0; i < login.length(); i++) {  // проверка отсутствия кириллицы
            if (Character.UnicodeBlock.of(login.charAt(i)).equals(Character.UnicodeBlock.CYRILLIC)) throw new LoginValidationException();
        }

        for (int i = 0, count = 0; i < login.length(); i++) {  // проверка наличия нижнего подчеркивания
            if (Character.valueOf(login.charAt(i)).equals('_')) count++;
            if (i == login.length()-1) {
                if (count == 0) throw new LoginValidationException();
            }
        }

        for (int i = 0, count = 0; i < login.length(); i++) {  // проверка наличия цифр
            if (Character.isDigit(login.charAt(i))) count++;
            if (i == login.length()-1) {
                if (count == 0) throw new LoginValidationException();
            }
        }

        for (int i = 0, countUpper = 0, countLower = 0; i < login.length(); i++) {  // Проверка наличия нижнего и верхнего регистра
            if (Character.isUpperCase(login.charAt(i))) countUpper++;
            if (Character.isLowerCase(login.charAt(i))) countLower++;
            if (i == login.length()-1) {
                if (countUpper == 0) throw new LoginValidationException();
                if (countLower == 0) throw new LoginValidationException();
            }
        }

        if (login.length() > 20) throw new LoginValidationException();  // Проверка корректной длинны логина
    }
    ...
    public static Boolean isLoginValid(String login) {
        try {
            validateLogin(login);
            return true;
        } catch (LoginValidationException loginException) {
            return false;
        }
    }
